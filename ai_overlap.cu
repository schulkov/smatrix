...
__global__ void overlap_primitive_ss(REAL_T *sab, REAL_T zeta, REAL_T zetb, REAL_T rab_x, REAL_T rab_y, REAL_T rab_z)
{
...
__global__ void overlap_primitive_sp(REAL_T *sab, REAL_T zeta, REAL_T zetb, REAL_T rab_x, REAL_T rab_y, REAL_T rab_z)
{
   REAL_T dab = sqrt(rab_x*rab_x + rab_y*rab_y + rab_z*rab_z);
...
__global__ void overlap_primitive_ps(REAL_T *sab, REAL_T zeta, REAL_T zetb, REAL_T rab_x, REAL_T rab_y, REAL_T rab_z)
{
   REAL_T dab = sqrt(rab_x*rab_x + rab_y*rab_y + rab_z*rab_z);
...
__global__ void overlap_primitive_pp(REAL_T *sab, REAL_T zeta, REAL_T zetb, REAL_T rab_x, REAL_T rab_y, REAL_T rab_z)
{
   REAL_T dab = sqrt(rab_x*rab_x + rab_y*rab_y + rab_z*rab_z);
                    ...
                    const REAL_T *gcc_b, REAL_T rab_x, REAL_T rab_y, REAL_T rab_z)
{
   ...
   sab_pgf = (REAL_T*) malloc(ncoa*ncob*sizeof(REAL_T));
   cudaError_t cuda_err = cudaMalloc( (void**) &sab_pgf_dev, ncoa*ncob*sizeof(REAL_T) );
   if (sab_pgf == NULL) return;
   memset(sab, 0, ncoa*ncob*sizeof(REAL_T));
   ...
   for (unsigned int ipgf_b = 0; ipgf_b < npgf_b; ++ipgf_b) {
      for (unsigned int ipgf_a = 0; ipgf_a < npgf_a; ++ipgf_a) {
          if (la_set == 0 && lb_set == 0) {
             overlap_primitive_ss<<<1,1>>>(sab_pgf_dev, zet_a[ipgf_a], zet_b[ipgf_b], rab_x, rab_y, rab_z);
          } else if (la_set == 0 && lb_set == 1) {
             overlap_primitive_sp<<<1,1>>>(sab_pgf_dev, zet_a[ipgf_a], zet_b[ipgf_b], rab_x, rab_y, rab_z);
          } else if (la_set == 1 && lb_set == 0) {
             overlap_primitive_ps<<<1,1>>>(sab_pgf_dev, zet_a[ipgf_a], zet_b[ipgf_b], rab_x, rab_y, rab_z);
          } else if (la_set == 1 && lb_set == 1) {
             overlap_primitive_pp<<<1,1>>>(sab_pgf_dev, zet_a[ipgf_a], zet_b[ipgf_b], rab_x, rab_y, rab_z);
          }
          cuda_err = cudaMemcpy( sab_pgf, sab_pgf_dev, ncoa*ncob*sizeof(REAL_T), cudaMemcpyDeviceToHost );

          for (unsigned int icob = 0; icob < ncob; ++icob) {
             for (unsigned int icoa = 0; icoa < ncoa; ++icoa) {
//                printf("B %d %d %d %f \n", cuda_err, icob, icoa, sab_pgf[icob*ncoa+icoa]);
